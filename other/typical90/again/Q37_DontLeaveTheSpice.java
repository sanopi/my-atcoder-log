...
public class Q37_DontLeaveTheSpice {
    ...
    public static void main(String[] args) {
        ...
        MaxSegTree maxSegTree = new MaxSegTree(dp[0]);
        ...
        for (int i = 1; i < n; i++) {
            ...
            for (int j = 0; j <= w; j++) {
                ...
                long max = maxSegTree.query(Math.max(0, j - r), j - l + 1);
                if (max < 0) continue;
                dp[i][j] = Math.max(dp[i][j], max+v);
//                for (int k = Math.max(0, j-r); k <= j-l; k++) {
//                    if (dp[i-1][k] < 0) continue;
//                    dp[i][j] = Math.max(dp[i][j], dp[i-1][k]+v);
//                }
            }
            maxSegTree = new MaxSegTree(dp[i]);
        }
