...
public class Q37_DontLeaveTheSpice_5 {
    ...
    public static void main(String[] args) {
        ...
        MaxSegTree[] z = new MaxSegTree[n+1];
        for (int i = 0; i <= n; i++) {
            z[i] = new MaxSegTree(dp[i]);
        }
        ...
        for (int i = 1; i <= n; i++) {
            ...
            // i番目の料理を作る
            for (int j = l; j <= w; j++) {
                int cl = Math.max(0, j - r);
                int cr = Math.max(0, j - l + 1);
                if (cr == 0) { continue; }
                long max = z[i - 1].query(cl, cr);
                if (!(max < 0)) {
            ...
            for (int j = 0; j <= w; j++) {
                z[i].updateValue(j, dp[i][j]);
            }
        }

        out.println(dp[n][w] < 0 ? -1 : dp[n][w]);
        out.flush();
    }
